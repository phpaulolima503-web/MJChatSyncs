  ...
  if (!quote) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-background text-muted-foreground p-4">
        ...
        <h2 className="text-lg font-bold text-foreground mb-2">Quotation Unavailable</h2>
      ...
      <div className="max-w-4xl mx-auto px-4 pt-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between border-b border-border/80 pb-6 gap-4">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 bg-indigo-650 flex items-center justify-center rounded-xl font-black text-foreground text-lg tracking-wider border border-indigo-500/20">
              MJ
            </div>
            <div>
              <h2 className="text-lg font-bold tracking-tight text-foreground">MaaJanki Web Tech</h2>
              <p className="text-xs text-muted-foreground">GSTIN: 09AAPCM9422H1Z5 • State Code: 09 (UP)</p>
            </div>
          </div>
        ...
        {/* GST Invoice Details */}
        <Card className="bg-card/70 border-border backdrop-blur-sm shadow-xl">
          <CardHeader className="p-6 border-b border-border bg-muted/40">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
              <div>
                ...
                <CardTitle className="text-lg text-foreground font-extrabold">INV-{quote.id.slice(0, 8).toUpperCase()}</CardTitle>
                <CardDescription className="text-muted-foreground text-xs mt-0.5">
                  Date: {new Date(quote.created_at).toLocaleDateString()}
                </CardDescription>
              </div>

              <div className="flex items-start gap-8 text-xs text-muted-foreground">
                <div>
                  <span className="font-bold text-muted-foreground block uppercase text-[9px]">Billed From</span>
                  <p className="font-semibold text-foreground mt-1">MaaJanki Web Tech</p>
                  <p className="text-[11px] mt-0.5">Varanasi, Uttar Pradesh, India</p>
                </div>
                <div>
                  <span className="font-bold text-muted-foreground block uppercase text-[9px]">Billed To</span>
                  <p className="font-semibold text-foreground mt-1">{quote.contacts?.name || "Client"}</p>
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="overflow-x-auto border border-border rounded-xl bg-muted/40 shadow-inner">
              <table className="w-full text-left text-foreground text-xs">
                <thead>
                  <tr className="border-b border-border bg-background/30 text-muted-foreground font-bold uppercase text-[9px] tracking-wider">
                ...
                <tbody className="divide-y divide-slate-850/60 font-medium">
                  {quote.items.map((item, idx) => (
                    <tr key={idx} className="hover:bg-card/10 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-right text-foreground font-mono">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Signature Card */}
          <Card className="bg-card/70 border-border backdrop-blur-sm flex flex-col justify-between">
            ...
            <CardContent className="p-5 pt-0">
              {isSigned ? (
                <div className="bg-muted/50 border border-border/60 rounded-xl p-4 text-center space-y-2 mt-2">
                  <span className="text-[9px] uppercase font-bold text-muted-foreground block">Digital Signature Verified</span>
                ...
                <form onSubmit={handleSignQuote} className="space-y-4 mt-2">
                  <div className="space-y-1.5">
                    <Label htmlFor="sig-input" className="text-xs text-muted-foreground">Type Your Full Name</Label>
                    <Input
                      id="sig-input"
                      value={signatureText}
                      ...
                      required
                      className="bg-background border-border text-foreground placeholder-slate-700 font-serif italic text-base text-center"
                    />
                  </div>
                  <Button
                    type="submit"
                    disabled={signing}
                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-foreground text-xs font-bold"
                  >
                    {signing ? (
                      <>
          ...
          {/* Payment Card */}
          <Card className={`bg-card/70 border-border backdrop-blur-sm transition-all duration-300 ${!isSigned && "opacity-40 pointer-events-none"}`}>
            <CardHeader className="p-5 pb-3">
              <CardTitle className="text-sm font-bold text-foreground flex items-center gap-2">
              ...
              {isPaid ? (
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-5 text-center space-y-2 mt-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">Verification synced to MaaJanki CRM pipeline.</p>
                  </div>
                ...
                <div className="space-y-4 mt-2">
                  {/* Tab selectors */}
                  <div className="flex items-center gap-2 bg-background p-1 rounded-lg border border-border w-fit mx-auto">
                    <Button 
                  ...
                  {activePaymentMethod === "upi" ? (
                    <div className="flex flex-col items-center justify-center p-3 border border-border bg-background/30 rounded-xl space-y-2">
                      ...
                      <span className="text-[10px] text-muted-foreground text-center leading-normal">
                        Scan with GPay, PhonePe, or Paytm to pay <strong className="text-foreground">₹{quote.total_amount.toLocaleString("en-IN")}.00</strong>
                      </span>
                    </div>
                  ) : (
                    <div className="p-4 border border-border bg-muted/40 rounded-xl space-y-3">
                      <div className="flex items-center justify-between border-b border-border pb-2">
                        <span className="text-[10px] text-muted-foreground font-bold uppercase">Stripe Gateway</span>
                        <Badge className="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-[9px] px-1.5">Sandbox Mode</Badge>
                      </div>
                      <div className="space-y-1">
                        <div className="h-7 bg-card border border-border rounded px-2.5 flex items-center text-xs text-muted-foreground">
                          4242 •••• •••• 4242
                        </div>
                      </div>
                    </div>
                  ...
                  <Button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-foreground font-bold h-9 shadow-lg shadow-emerald-900/10"
                  >
                    {paying ? (
                      <>
