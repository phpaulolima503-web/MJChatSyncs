...
export default function PlaceholderPage({ title, desc, icon: Icon }: { title: string, desc: string, icon: any }) {
  return (
    <div className="p-6 h-[calc(100vh-2rem)] flex flex-col items-center justify-center text-center">
      <div className="rounded-full bg-card p-6 mb-6">
        <Icon className="h-16 w-16 text-muted-foreground" />
      </div>
      <h1 className="text-2xl font-bold text-foreground mb-2">{title}</h1>
      <p className="text-muted-foreground max-w-md">{desc}</p>
      <button className="mt-8 rounded-lg bg-muted px-6 py-2.5 text-sm font-medium text-foreground hover:bg-muted">Coming in next update</button>
    </div>
  )
}
