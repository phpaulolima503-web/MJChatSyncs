    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Analyze sales pipelines, deal sizes, and employee sales performance.</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-xl border border-border bg-card/70 p-5">
          <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Total Pipeline Value</span>
          <div className="mt-2 text-2xl font-bold text-foreground">
            ₹{pipelineData.totalPipelineValue.toLocaleString()}
          </div>
          <span className="text-xs text-muted-foreground">Value of all open and won deals</span>
        </div>

        <div className="rounded-xl border border-border bg-card/70 p-5">
          <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Average Deal Size</span>
          <div className="mt-2 text-2xl font-bold text-foreground">
        ...
        <div className="rounded-xl border border-border bg-card/70 p-5">
          <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Sales Conversion Rate</span>
          <div className="mt-2 text-2xl font-bold text-foreground">
            {pipelineData.conversionRate}%
          </div>
          <span className="text-xs text-muted-foreground">Leads to won deals ratio</span>
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm text-foreground">
                <span>New Leads</span>
                <span className="font-semibold text-foreground">{pipelineData.newLeads}</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm text-foreground">
                ...
                <span className="font-semibold text-foreground">{pipelineData.qualifiedLeads}</span>
              </div>
              <div className="h-3 w-full rounded-full bg-muted overflow-hidden">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm text-foreground">
                <span>Won Deals</span>
                <span className="font-semibold text-foreground">{pipelineData.wonDeals}</span>
              </div>
              <div className="h-3 w-full rounded-full bg-muted overflow-hidden">
            ...
            {revenueByEmployee.map((emp, idx) => (
              <div key={emp.name} className="py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-7 w-7 rounded-full bg-background flex items-center justify-center text-xs font-bold text-muted-foreground border border-border">
                    {idx + 1}
                  </div>
                  <div>
                    <div className="text-sm font-medium text-foreground">{emp.name}</div>
                    <div className="text-xs text-muted-foreground">{emp.deals} deals closed</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-bold text-foreground">₹{emp.revenue.toLocaleString()}</div>
