  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground">New GST Invoice</h1>
        <div className="flex gap-2">
          <button onClick={() => save(false)} disabled={saving} className="flex items-center gap-2 rounded-lg border border-border px-4 py-2 text-sm text-foreground hover:border-slate-500 disabled:opacity-50">
            <Save className="h-4 w-4" /> Save Draft
          </button>
          <button onClick={() => save(true)} disabled={saving} className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-foreground hover:bg-primary/90 disabled:opacity-50">
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">Invoice Date</label>
          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-border/50">
              {items.map((item, i) => {
                ...
                return (
                  <tr key={i}>
                    <td className="py-2 pr-2"><input value={item.description} onChange={e => updateItem(i, "description", e.target.value)} placeholder="Description" className="w-full bg-transparent text-foreground placeholder-slate-700 focus:outline-none" /></td>
                    <td className="py-2 pr-2"><input value={item.hsn_sac} onChange={e => updateItem(i, "hsn_sac", e.target.value)} placeholder="HSN/SAC" className="w-full bg-transparent text-muted-foreground placeholder-slate-700 focus:outline-none" /></td>
                    <td className="py-2 pr-2 text-right"><input type="number" min="0" value={item.quantity} onChange={e => updateItem(i, "quantity", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-right text-foreground focus:outline-none" /></td>
                    <td className="py-2 pr-2"><select value={item.unit ?? "NOS"} onChange={e => updateItem(i, "unit" as keyof LineItem, e.target.value)} className="bg-transparent text-muted-foreground focus:outline-none">{UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></td>
                    <td className="py-2 pr-2 text-right"><input type="number" min="0" value={item.rate} onChange={e => updateItem(i, "rate", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-right text-foreground focus:outline-none" /></td>
                    <td className="py-2 pr-2 text-right"><input type="number" min="0" max="100" value={item.discount_pct} onChange={e => updateItem(i, "discount_pct", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-right text-muted-foreground focus:outline-none" /></td>
                    <td className="py-2 pr-2 text-right">
                      <select value={item.tax_rate} onChange={e => updateItem(i, "tax_rate", parseInt(e.target.value))} className="bg-transparent text-muted-foreground focus:outline-none">
                        {TAX_SLABS.map(r => <option key={r} value={r}>{r}%</option>)}
                      </select>
                    </td>
                    <td className="py-2 pr-2 text-right font-medium text-foreground">{INR(calcItem?.total_amount ?? 0)}</td>
                    <td className="py-2">
                      {items.length > 1 && <button onClick={() => removeItem(i)} className="text-muted-foreground hover:text-red-400"><Trash2 className="h-3 w-3" /></button>}
                    </td>
                  </tr>
                )
              })}
        ...
        <div className="border-t border-border pt-4">
          <div className="ml-auto max-w-xs space-y-1.5 text-xs">
            {[
              ...
              <div key={r.label} className="flex justify-between text-muted-foreground"><span>{r.label}</span><span>{r.value}</span></div>
            ))}
            <div className="flex justify-between border-t border-border pt-2 text-base font-bold text-foreground">
              <span>Total</span><span>{INR(calc.grand_total)}</span>
            </div>
            <p className="text-[10px] text-muted-foreground italic">{calc.amount_in_words}</p>
          </div>
        </div>
      </div>
      ...
      {/* Notes */}
      <div className="rounded-xl border border-border bg-card p-5">
        <label className="mb-2 block text-xs text-muted-foreground">Notes / Terms</label>
        <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} rows={3} placeholder="Payment terms, bank details, thank you note..." className="w-full rounded-lg border border-border bg-muted px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none" />
      </div>
    </div>
  )
}
