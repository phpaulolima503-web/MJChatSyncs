  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Lead Scoring & Intelligence</h1>
          <p className="text-sm text-muted-foreground mt-0.5">AI-powered lead qualification and conversion prediction</p>
        </div>
        <button onClick={scoreAll} disabled={scoring} className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-foreground hover:bg-primary/90 disabled:opacity-50">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { label: 'Total Scored', value: scores.length, icon: Users, color: 'text-muted-foreground' },
      ...
      <div className="rounded-xl border border-border bg-card overflow-x-auto">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <h2 className="text-sm font-semibold text-foreground">Top Leads by Score</h2>
          <button onClick={load} className="text-muted-foreground hover:text-foreground"><RefreshCw className={cn("h-3.5 w-3.5", loading && "animate-spin")} /></button>
        </div>
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border/50">
            {scores.map(s => (
              <tr key={s.id} className="hover:bg-muted/30">
                <td className="px-4 py-3">
                  <p className="font-medium text-foreground">{s.contacts?.name ?? '—'}</p>
                  <p className="text-xs text-muted-foreground">{s.contacts?.company ?? s.contacts?.email}</p>
                </td>
                <td className="px-4 py-3 text-center">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-xs font-bold text-foreground">{s.total_score}</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-center">
                  <span className={cn("rounded border px-2 py-0.5 text-xs font-bold", GRADE_COLORS[s.score_grade] ?? '')}>{s.score_grade}</span>
                </td>
                <td className="px-4 py-3 text-center">{s.is_mql ? <span className="text-blue-400 text-xs">✓ MQL</span> : <span className="text-muted-foreground text-xs">—</span>}</td>
                <td className="px-4 py-3 text-center">{s.is_sql ? <span className="text-emerald-400 text-xs">✓ SQL</span> : <span className="text-muted-foreground text-xs">—</span>}</td>
                <td className="px-4 py-3 text-right text-sm font-semibold text-foreground">{Math.round(s.conversion_probability ?? 0)}%</td>
            ...
            {scores.length === 0 && !loading && <tr><td colSpan={7} className="py-12 text-center text-muted-foreground">No scored leads yet. Click "Score All" to start.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
