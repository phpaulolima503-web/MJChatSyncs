    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Manage WhatsApp and manual orders</p>
        </div>
        <div className="flex gap-2">
          <Link href="/commerce/products" className="flex items-center gap-2 rounded-lg border border-border px-4 py-2 text-sm text-muted-foreground hover:text-foreground">Catalog</Link>
          <button className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-foreground hover:bg-primary/90">New Order</button>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search order #, customer, phone..." className="w-full rounded-lg border border-border bg-muted pl-8 pr-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="rounded-lg border border-border bg-muted px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none">
      ...
      <div className="rounded-xl border border-border bg-card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border/50">
            {filtered.map(o => (
              <tr key={o.id} className="hover:bg-muted/30">
                <td className="px-4 py-3">
                  ...
                  <p className="text-[10px] text-muted-foreground">{o.commerce_order_items?.length} items</p>
                </td>
                <td className="px-4 py-3 text-xs text-muted-foreground">{new Date(o.created_at).toLocaleDateString('en-IN')}</td>
                <td className="px-4 py-3">
                  <p className="font-medium text-foreground">{o.contacts?.name ?? '—'}</p>
                  <p className="text-xs text-muted-foreground">{o.contacts?.phone}</p>
                </td>
                <td className="px-4 py-3 text-center">
                  <span className={cn("rounded border px-2 py-0.5 text-xs font-bold capitalize", STATUS_COLORS[o.status] ?? '')}>{o.status}</span>
                </td>
                <td className="px-4 py-3 text-center">
                  <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize", o.payment_status === 'paid' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-muted text-muted-foreground')}>{o.payment_status}</span>
                </td>
                <td className="px-4 py-3 text-right font-bold text-foreground">{INR(o.total)}</td>
            ...
            {filtered.length === 0 && !loading && <tr><td colSpan={7} className="py-12 text-center text-muted-foreground">No orders found</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
