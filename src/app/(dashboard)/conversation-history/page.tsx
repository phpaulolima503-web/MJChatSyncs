...
const STATUS_COLORS: Record<ConversationStatus, string> = {
  ...
  closed: "bg-slate-600/20 text-muted-foreground border-border/30",
};
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card/60 p-3">
        <div className="relative min-w-[220px] flex-1">
          ...
          <Input
            ...
            className="border-border bg-muted pl-9 text-sm text-foreground placeholder:text-muted-foreground"
          />
        </div>

        <DropdownMenu>
          <DropdownMenuTrigger className="inline-flex items-center gap-1.5 rounded-md border border-border bg-muted px-3 py-2 text-xs font-medium text-foreground hover:bg-accent">
            <User className="h-3.5 w-3.5" />
            {activeAgentLabel}
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="max-h-[280px] overflow-y-auto border-border bg-muted">
            ...
            {profiles.map((p) => (
              <DropdownMenuItem
                ...
                className={cn("text-xs", agentFilter === p.user_id ? "text-primary font-bold" : "text-foreground")}
              >
                {p.full_name}
              </DropdownMenuItem>
            ))}
        ...
        <div className="relative">
          ...
          <input
            ...
            className="rounded-md border border-border bg-muted py-2 pl-8 pr-2 text-xs text-foreground [color-scheme:dark]"
          />
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card/60">
        {loading ? (
          ...
          <div className="px-4 py-16 text-center">
            <p className="text-sm text-muted-foreground">Nenhuma conversa encontrada com esses filtros</p>
          </div>
        ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-foreground">
                        {agent ?? <span className="text-muted-foreground">Não atribuído</span>}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={cn(
