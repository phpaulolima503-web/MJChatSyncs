    ...
    <div className="space-y-4">
      ...
      {showCreate && (
        <div className="rounded-xl border border-border bg-muted p-4 space-y-3">
          <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Report name..." className="w-full rounded-md border border-border bg-muted px-3 py-2 text-sm text-foreground" />
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-muted-foreground">Type</label>
              <select value={form.report_type} onChange={e => setForm(p => ({ ...p, report_type: e.target.value }))} className="mt-1 w-full rounded-md border border-border bg-muted px-2 py-1.5 text-xs text-foreground">
            ...
            <div>
              ...
              <select value={form.export_format} onChange={e => setForm(p => ({ ...p, export_format: e.target.value }))} className="mt-1 w-full rounded-md border border-border bg-muted px-2 py-1.5 text-xs text-foreground">
          ...
          <div className="flex gap-2">
            <button onClick={create} className="flex-1 rounded-md bg-primary py-2 text-xs font-medium text-foreground hover:bg-primary/90">Create</button>
            <button onClick={() => setShowCreate(false)} className="rounded-md border border-border px-3 py-2 text-xs text-muted-foreground hover:text-foreground">Cancel</button>
          </div>
        </div>
      )}
      ...
      {reports.length === 0 && !showCreate && (
        <div className="rounded-xl border border-border bg-card p-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">No saved reports yet</p>
        </div>
      )}
          ...
          <div key={r.id} className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground capitalize">{r.report_type} · {r.export_format} · {r.generate_count} exports</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-border px-3 py-1.5 text-xs text-foreground hover:border-primary hover:text-primary transition-colors disabled:opacity-50"
              >
