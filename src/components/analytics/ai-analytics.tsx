          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-border/50">
              {(ai.by_provider || []).map(p => (
                <tr key={p.provider}>
                  <td className="py-2 text-foreground font-medium capitalize">{p.provider}</td>
                  <td className="py-2 text-right text-muted-foreground">{p.requests.toLocaleString()}</td>
                  <td className="py-2 text-right text-muted-foreground">{p.tokens.toLocaleString()}</td>
                  <td className="py-2 text-right text-muted-foreground">${p.cost_usd.toFixed(4)}</td>
                  <td className="py-2 text-right text-muted-foreground">{p.avg_latency_ms}ms</td>
            ...
            {(ai.daily_usage || []).slice(-30).map(d => {
              ...
              return (
                <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group relative">
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover:block bg-muted text-foreground text-[10px] px-2 py-1 rounded whitespace-nowrap z-10">
                    {d.date}: {d.requests}
                  </div>
