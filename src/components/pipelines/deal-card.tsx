...
export function DealCard({ deal, stage, onEdit, isOverlay }: DealCardProps) {
  ...
  return (
    <button
      ...
      className={`group relative w-full cursor-pointer rounded-xl border border-border/50 bg-accent pl-4 pr-3 py-3 text-left shadow-sm transition-all ${
        isOverlay
          ? "shadow-xl"
    ...
    >
      ...
      {deal.services && deal.services.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {deal.services.map((svc) => (
            <span key={svc} className="inline-flex items-center rounded bg-muted/70 px-1.5 py-0.5 text-[9px] font-medium text-foreground">
              {svc}
            </span>
          ))}
        </div>
      ...
      {!isOverlay && deal.contact_id && (
        <div className="mt-2.5 pt-2 border-t border-border/60 flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <Link href={`/contacts/${deal.contact_id}`} onClick={(e) => e.stopPropagation()} className="shrink-0">
            <span className="inline-flex items-center gap-1 rounded bg-muted/55 hover:bg-muted hover:text-foreground px-2 py-0.5 text-[9px] font-bold text-foreground transition-colors">
